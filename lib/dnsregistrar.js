        ...
        return {
            ...
            submit: async (params = {}) => {
                if(proveResult.nsec){
                    ...
                    registrar.methods.claim(window.encodedName, window.empty)
                    // Need to remove trailing '.'. Otherwise the transaction fails.
                    let deleteName = encodedName.slice('0',-2);
                    // Anyone can put empty byte if no record on DNSSEC Oracle
                    let proof = [];
                    await this.registrar.methods.claim(deleteName, proof).send(params);
